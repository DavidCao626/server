...
use App\EofficeApp\Calendar\Entities\calendarPurviewManageUserEntity;
 ...
 *
 */
class CalendarSharePurviewManageUserRepository extends BaseRepository 
{
    ...
    public function __construct(calendarPurviewManageUserEntity $entity) 
    {
 ...
 /**
    ...
    public function getInitListSelector($param, $userId)
    {
        ...
        $param['order_by'] = empty($param['order_by']) ? $this->orderBy : $param['order_by'];
        $query = $query->where('repeat_remove', '=', 0)->orders($param['order_by'])->where('creator', $userId);
