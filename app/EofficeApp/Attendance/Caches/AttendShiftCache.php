 ...
 */
class AttendShiftCache extends AttendanceCache implements ECacheInterface
{
    // 默认是string数据结构,当前缓存实体使用的是hash数据结构
    public $struct = 'string';
    // 统一使用大写字母加下划线（_）;
    public $key = 'ATTEND_SHIFT';
    // 该缓存实体描述
    public $description = '用于缓存班次信息';
    // 缓存实体被调用的时机
    public $called = [
        // 设置
        'set' => [
    ...
    public function get($dynamicKey = null)
    {
        return $this->find($dynamicKey, function($dynamicKey = null) {
            return app('App\EofficeApp\Attendance\Repositories\AttendanceShiftsRepository')->getDetail($dynamicKey);
        });
    }
}
