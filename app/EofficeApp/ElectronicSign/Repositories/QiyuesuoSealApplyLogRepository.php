...
use App\EofficeApp\ElectronicSign\Entities\QiyuesuoSealApplyLogEntity;

class QiyuesuoSealApplyLogRepository extends BaseRepository
{
    public function __construct(QiyuesuoSealApplyLogEntity $entity)
    {
    ...
    public function getList($param = [])
    {
        $default = [
            ...
            'order_by' => ['id' => 'desc'],
            'limit' => 10,
            'fields' => ['qiyuesuo_seal_apply_log.*'],
        ];
        ...
        $query->AddSelect(['user.user_name as creator_name', 'user.created_at as user_created_at'])->leftJoin('user', 'user.user_id', '=', 'qiyuesuo_seal_apply_log.userid');
        $query = $this->getParseWhere($query, $param);
        ...
        if ($search) {
            if (isset($search['run_name'])) {
                ...
                $query = $query->leftJoin('flow_run', function($join) use ($runName) {
                    $join->on('flow_run.run_id', '=', 'qiyuesuo_seal_apply_log.runId');
                })
