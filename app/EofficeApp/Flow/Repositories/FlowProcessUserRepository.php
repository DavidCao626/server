...
use App\EofficeApp\Flow\Entities\FlowProcessUserEntity;
 ...
 * @since  2015-10-16 创建
 */
class FlowProcessUserRepository extends BaseRepository
{
    public function __construct(FlowProcessUserEntity $entity)
    {
    ...
    {
        $query = $this->entity;
        $query = $query->leftJoin('flow_process', 'flow_process.node_id', '=', 'flow_process_user.id');
        $query = $query->leftJoin('user_system_info', 'user_system_info.user_id', '=', 'flow_process_user.user_id');
        $query = $query->leftJoin('user', 'user.user_id', '=', 'user_system_info.user_id')
        ->select(['user.user_name','flow_process_user.user_id','flow_process_user.id as node_id'])
    ...
    function getFlowHandleUserList($id)
    {
        $query = $this->entity;
        $query = $query->leftJoin('flow_process', 'flow_process.node_id', '=', 'flow_process_user.id');
        $query = $query->leftJoin('user', 'user.user_id', '=', 'flow_process_user.user_id')
        ->select(['user.user_name','flow_process_user.user_id','flow_process_user.id as node_id'])
        ->where("flow_process.flow_id",$id);
