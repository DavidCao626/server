<?php
namespace App\EofficeApp\FormModeling\Permissions;
class FormModelingPermission
{
    ...
    public function getCustomDataDetail($own, $data, $urlData)
    {
        ...
        if(!isset($urlData['tableKey']) || empty($urlData['tableKey'])) {
            return ['code' => ['0x009003', 'menu']];
        }
        $tableKey = $urlData['tableKey'];
        return app($this->formModelingService)->checkPermission($tableKey,'detailMenu');
    }
    // /**
    //  * 验证删除数据菜单权限
    //  * @param type $own // 当前用户信息
