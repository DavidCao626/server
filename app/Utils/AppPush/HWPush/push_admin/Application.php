...
class Application
{
    ...
    {
        $this->appid = str_replace(array("\r\n", "\r", "\n"), "", $appid);
        $this->appsecret = str_replace(array("\r\n", "\r", "\n"), "", $appsecret);
        $this->hw_token_server = str_replace(array("\r\n", "\r", "\n"), "", $hw_token_server);
        $this->hw_push_server = str_replace(array("\r\n", "\r", "\n"), "",  $hw_push_server);
        $this->redisClient = new \Predis\Client([
            'password' => envOverload('REDIS_PASSWORD', 'eoffice10redis'),
            'port' => envOverload('REDIS_PORT', 6379),
            'host' => envOverload('REDIS_HOST', '127.0.0.1'),
        ]);
        $this->token_expiredtime = $this->redisClient->get('huawei_token_expiredtime') ?? null;
        $this->accesstoken = $this->redisClient->get('huawei_accesstoken') ?? null;
    ...
    private function refresh_token()
    {
        ...
        $this->redisClient->set('huawei_accesstoken', $this->accesstoken);
        $this->redisClient->set('huawei_token_expiredtime', $this->token_expiredtime);
